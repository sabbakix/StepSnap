...
// Stato Vista Scorrimento
let scrollViewMode = false;
...
function setupEventListeners() {
  ...
  document.getElementById('btnToggleScrollView').addEventListener('click', () => {
    if (scrollViewMode) exitScrollView(); else enterScrollView();
  });
...
async function loadDashboard() {
  ...
  allSessions = await dbGetAllSessions();
  renderGuideCards(allSessions);
}

function renderGuideCards(sessions, query = '') {
...
function openWorkspace() {
  if (scrollViewMode) exitScrollView(true);
...
function enterScrollView() {
  ...
  const firstId = selectedStepId || currentSession.steps[0].id;
  highlightSidebarStep(firstId);
}

function exitScrollView(silent = false) {
...
function onScrollViewInput(e) {
  ...
  if (!stepEl) return;
  const step = currentSession.steps.find(s => s.id === stepEl.dataset.stepId);
  if (!step) return;
...
function highlightSidebarStep(id) {
...
function openSvImageEditor(stepId) {
  ...
  const idx = currentSession.steps.indexOf(step);
